  ...
  const usersElement = users.map((user) => {
    const confirmAction = () => {
      if (window.confirm("Delete Account?")) {
        axios.post(
          "http://localhost:80/rbimv5/server/Accounts_Delete.php",
          user
        );
      ...
      return (
        <button
          ...
          onClick={(e) => {
            e.preventDefault();
            axios.post(
              "http://localhost:80/rbimv5/server/Accounts.php",
              inputs
            );
      ...
      return (
        <button
          ...
          onClick={(e) => {
            e.preventDefault();
            axios.post(
              "http://localhost:80/rbimv5/server/Accounts_SignUp.php",
              inputs
            );
  ...
  return ReactDom.createPortal(
    <>
      ...
      <div className="Accounts__modal">
        <button onClick={onClose} className="Accounts__modal__close__btn">
          &#10006;
        </button>
        <h1>Accounts</h1>
